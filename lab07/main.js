...
function runSimulation(isAnimation) {
	...
	if (!isAnimation) {
		while (currentTime < maxTime) {
			...
			simulationData.push({
				dayEnd: currentTime.toFixed(2),
				state: stateNames[currentState],
				duration: stepTime.toFixed(2),
			});
...
function saveCSV() {
	...
	csv += "Состояние,Время (дни),Эмп. вер.,Теор. вер.,Погрешность\n";
	...
	// Добавляем BOM (\uFEFF) для того, чтобы Excel корректно распознавал кириллицу (UTF-8)
